...
public class DogRegister {
	...
	private boolean dogSetter(String toOwner, String fromOwner, String dogName) {
		...
		for (Dog dog : dogCollection()) {
			if (dog.getName().equals(formatter.format(dogName))) {
	...
	private boolean addDog() {
		...
		boolean result = ownerCollection.getOwner(ownerName).addDog(dog);
		if (!result) {
			printErrorMessage("Error enter a number");
		}
		return result;
	}
	...
	private boolean printList(Dog[] dogs, double minimumTailLength) {
		...
		for (Dog dog : dogs) {
			if (dog.getTailLength() >= minimumTailLength) {
				System.out.println(
						dog.getName() + "\t\t|\t" + dog.getTailLength() + "\t\t|\t" + dog.getOwner().getName());
				hasPrinted = true;
			}
		}
		return hasPrinted;
	...
	private Dog dogCreator() {
		// try {
		Dog newDog = new Dog(input.scanString("enter dog name:"), input.scanString("enter dog breed:"),
				input.scanInt("enter dog age:"), input.scanDouble("enter dog weight:"));
		return newDog;
		...
		// return null;
		// }
		// commented code is here as a input sanitazion attempt. it fails style checks.
	}
